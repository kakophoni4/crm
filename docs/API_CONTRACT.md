...
## Карточки лавок и ОКВЭД заявки

`GET /api/v1/accounting/assignments/units` доступен с `accounting.read`: бухгалтер получает свои лавки, главный бухгалтер/администратор — все. Управление назначениями остаётся с `accounting.manage`.

`GET /api/v1/accounting/units/{unit_id}/card` возвращает общую карточку лавки либо `null`, если карточка ещё не создана. `PATCH` по тому же адресу принимает только изменённые поля; `null` очищает значение. Обе операции проверяют назначение по ИНН лавки, включая неактивные лавки. Главный бухгалтер и администратор имеют доступ ко всем. Дата приёма — `received_at` (ISO date); дировод — `dirovod` связанного директора. Назначенный бухгалтер меняется через API назначений. Название синхронизируется с `opt_units`. Операционные поля показываются юристу из той же записи.

`PATCH /api/v1/leads/{lead_id}/opt-orders/{order_id}/okved` принимает `{ "buyer_okved": "47.11" }` и возвращает это поле. Требуется `contacts.update` и доступ к сделке/заявке. Поле необязательное (до 200 символов), пробелы по краям убираются, пустое значение сохраняется как `null`. Изменение касается только выбранной заявки. `OptOrderResponse.buyer_okved` и совместимое поле реестра оплат `client_okved` читаются из `lead_opt_orders.buyer_okved`, не из контакта.

Для обновления схемы требуется миграция `0117_order_buyer_okved`. Старые значения ОКВЭД контактов намеренно не копируются в заявки: у контакта могут быть разные покупатели.


### Период квитанций СБИС

При приёме квитанции распознанный из PDF отчётный квартал имеет приоритет перед параметром `period_code` загрузчика. Поддерживается формат «1 квартал, 21, 2026 год». Если период корректировки не распознан, API возвращает ошибку валидации; период партии не подставляется. Для первичных документов параметр `period_code` остаётся запасным источником. При повторной загрузке метаданные `parsed` заменяются свежим результатом распознавания.

`scripts/repair_q1_2026_receipts.sql` исправляет только проверенные по оригинальным PDF записи 37 (7751377028) и 40 (9729323581) с `2/26` на `1/26`. Скрипт проверяет идентификаторы и текущие значения, сохраняет прежний период и метаданные распознавания в `metadata.period_repair`; повторный запуск ничего не изменяет.
