...
| **2+ групп** | Одна из назначенных групп (предпочитаются группы с доступными операторами); владелец — round-robin внутри выбранной группы | Операторы этой группы |
| **Ровно 1 группа** | Эта группа | Операторы группы + round-robin владельца |

Для теста с операторами: назначьте бота **ровно одной** группе, где есть пользователи с ролью `user` и доступностью **«Доступен»**. При нескольких группах обе (и больше) можно оставлять — чат не должен уходить в `__department_inbox__`, если у бота есть реальные группы.
