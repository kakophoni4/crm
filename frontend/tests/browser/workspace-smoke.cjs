...
async function main() {
  ...
  try {
    ...
    for (const theme of ['light', 'dark']) {
      ...
      assert.equal(await page.locator('.sales-register tbody tr').count(),1)
      ...
      await page.getByText('Заявок не найдено', {exact:true}).waitFor()
      await page.locator('.sales-register').getByRole('textbox').fill('')
      await page.locator('.sales-register tbody tr').first().waitFor()
