...
def _check_in_memory(user_id: int, limit: int) -> None:
    ...
    if len(window) >= limit:
        raise RateLimited(
            message="Contact update rate limit exceeded",
...
async def _check_redis(user_id: int, limit: int) -> None:
    redis = get_redis()
    key = f"contacts:rate:{user_id}".encode()
...
async def _enforce_contact_update_rate_limit(user_id: int) -> None:
    settings = get_settings()
    limit = settings.contacts_update_rate_limit_per_minute
    if limit <= 0:
        return

    if settings.contacts_update_rate_limit_use_redis:
        try:
            ...
            logger.warning("contact_update_rate_limit_redis_fallback", user_id=user_id)

    _check_in_memory(user_id, limit)


async def check_contact_update_rate_limit(
    actor: Annotated[User, Depends(requires_permission(Permission.CONTACTS_UPDATE))],
) -> User:
    await _enforce_contact_update_rate_limit(actor.id)
    return actor
