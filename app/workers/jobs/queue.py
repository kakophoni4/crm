...
STREAM_KEY = "crm:jobs"
DELAYED_KEY = "crm:jobs:delayed"
CONSUMER_GROUP = "crm-workers"
WORKER_NAME = "crm-worker-1"
...
async def _ensure_group() -> None:
    ...
    try:
        ...
        if "BUSYGROUP" not in str(exc):
            logger.debug("crm_jobs_group_exists", error=str(exc))
...
async def _process_message(raw: dict[bytes, bytes]) -> None:
    ...
    if handler is None:
        logger.warning("unknown_crm_job", job_type=job_type)
        return
            ...
            for _stream, entries in messages:
                for message_id, fields in entries:
                    try:
                        ...
                        logger.exception("crm_job_failed", message_id=message_id.decode())
        except asyncio.CancelledError:
            raise
        except Exception:
            logger.exception("crm_jobs_worker_loop_error")
...
def start_worker() -> None:
    ...
    if _worker_task is not None and not _worker_task.done():
        return
    _worker_task = asyncio.create_task(_worker_loop(), name="crm-jobs-worker")
